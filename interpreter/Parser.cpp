...
str_t Parser::next_token(size_t sym_pos) {
    ...
    if (next_token.empty()) throw InterpreterException("Missed src input for command!");
    curr_pos = next_pos;
    return next_token;
...
str_t Parser::parse_text(size_t sym_pos) {
    ...
    if (next_pos == input_line.length() - 1) {
        throw InterpreterException("Missed text!");
    } else if (!contains(input_line.substr(next_pos + 1), '"')) {
        throw InterpreterException("Missed bracket!");
    } else {
